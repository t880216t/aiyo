...
description: Use when creating or modifying UI components, styling, or visual elements related to Settings in AiYo.
license: MIT
...
## Purpose
This skill provides instructions for creating or redesigning Settings pages, informational panels, and configuration interfaces within the AiYo application.
...
Always utilize the standard AiYo typography classes defined in `packages/ui/src/lib/typography.ts`.
...
- **Reuse via composition**: Prefer a single settings component with a `visibleSettings` subset (like `AiYoVisualSettings`) for multiple tabs (Appearance/Chat) instead of duplicating markup.
