...
AiYo server process and forwards requests to a **loopback** target
(`localhost`, `127.0.0.1`, `::1`, `0.0.0.0`). It works for these topologies:

| Topology                                                                 | Works today? |
| ------------------------------------------------------------------------ | ------------ |
| Web UI in browser, AiYo server on same host as dev server         | yes          |
| Electron desktop, dev server on same host                                | yes          |
| VS Code extension, dev server on same host                               | yes          |
| Mobile/tablet hitting AiYo over LAN, dev server on host           | yes          |
| **Remote AiYo** (cloud / shared / tunneled), dev server on user's local machine | **no**       |

The blocked case is real: a user runs `aiyo serve` on a remote box (or a
hosted AiYo instance) but their dev server (`vite`, `next dev`, etc.)
...
that tunnels traffic from the remote AiYo back to the user's laptop
...
- Providing a hosted relay service. The relay is something the user runs;
  AiYo provides the agent + the server endpoints.

## Constraints (carried forward from the loopback proxy)

- Same-origin in the browser. The iframe must load from the AiYo
...
- Strip AiYo credentials before forwarding to the dev server.
...
- A subcommand of the existing CLI: `aiyo preview-agent`.
...
- Open exactly one outbound, authenticated WebSocket to the remote
  AiYo server (`wss://<host>/api/preview/agent`). Outbound-only — no
...
- TLS termination. The agent only talks to loopback over plain HTTP; the
  outbound link to AiYo is TLS via the server's existing cert.
- Anything that mutates the user's filesystem.
- Acting as a general SOCKS/HTTP proxy. It is dev-server-scoped.

### 2. Remote AiYo server (extends `proxy-runtime.js`)
...
2. Server mints a short-lived (5 min) enrollment token bound to the user's
   ...
   shows the command:
   ```
   aiyo preview-agent --server https://<host> --token <enrollment-token>
   ```
3. Agent posts the enrollment token to `POST /api/preview/agent/enroll` and
   ...
   config dir (`$XDG_CONFIG_HOME/aiyo/agent.json` or platform
   equivalent).
...
- **Dev-server credentials**: the agent strips `cookie`, `authorization`,
  and `x-aiyo-ui-session` before forwarding to the local dev
...
1. **CLI surface.** Is `aiyo preview-agent` the right verb, or should
   it live under `aiyo agent preview`? Bias: the former; only one
...
- **A reverse SSH tunnel from the agent.** Works but requires SSH server
  on the AiYo host, exposes a port, and breaks the same-origin
  guarantee unless we also reverse-proxy that port through the
  AiYo HTTP server. The control-WebSocket design avoids all of
  that and keeps a single TLS endpoint.
- **Cloudflare/ngrok-style hosted relay.** Would work but turns
  AiYo into a service that depends on a third party (or on us
