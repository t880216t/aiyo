...
Use this guide when running AiYo behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm AiYo works directly first.
...
- Disable gzip on the proxy if AiYo is already compressing responses.
...
- AiYo reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|aiyo/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- AiYo compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by AiYo, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
