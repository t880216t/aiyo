        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('AiYo startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'aiyo:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`AiYo server listening on ${bindHost}:${activePort}`);
