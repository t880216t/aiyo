export const registerAiYoRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/aiyo/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/aiyo/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(aiyoDataDir, 'run', `aiyo-${currentPort}.json`);
      ...
      let restartCmdFallback = `aiyo serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(aiyoDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== AiYo update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting AiYo...
              ${restartCmd || 'echo Service manager will restart AiYo.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/aiyo/models-metadata', async (_req, res) => {
    const now = Date.now();
