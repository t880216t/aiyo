...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/AiYo route wiring.
...
- `packages/web/server/lib/opencode/aiyo-routes.js`: AiYo update and models metadata route registration.
...
- `createCustomProviderConfig({ providerId, name, baseURL, models, scope, workingDirectory })`: Adds an OpenAI-compatible custom provider to user or project OpenCode config.
- `getProviderSources(providerId, workingDirectory)`: Returns provider config source presence across auth, user, project, and custom config layers.
- `removeProviderConfig(providerId, workingDirectory, scope)`: Removes provider config from a selected config layer.
...
## Public exports (aiyo-routes.js)
- `registerAiYoRoutes(app, dependencies)`: registers AiYo endpoints:
  - `GET /api/aiyo/update-check`
  - `POST /api/aiyo/update-install`
  - `GET /api/aiyo/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `AIYO_RATE_LIMIT_MAX_ATTEMPTS`, `AIYO_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
