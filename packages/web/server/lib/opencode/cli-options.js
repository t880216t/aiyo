export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.AIYO_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.AIYO_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.AIYO_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.AIYO_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.AIYO_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.AIYO_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.AIYO_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.AIYO_API_ONLY === '1' || env.AIYO_API_ONLY === 'true';
