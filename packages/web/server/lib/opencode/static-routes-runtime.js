...
export const createStaticRoutesRuntime = (dependencies) => {
  ...
  const resolveDistPath = () => {
    const env = typeof process.env.AIYO_DIST_DIR === 'string' ? process.env.AIYO_DIST_DIR.trim() : '';
    if (env) {
  ...
  const registerApiOnlyFallbackRoutes = (app) => {
    app.get(/^(?!\/api|\/auth|\/health|.*\.(js|css|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot|map)).*$/, (req, res) => {
      const command = 'aiyo connect-url --help';
...
<head>
  ...
  <title>AiYo API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="AiYo logo">
    ...
    <h1>AiYo is running in headless mode</h1>
    <p>This server is ready. Open it from the AiYo desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'AiYo is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('AiYo is running in API-only mode');
        },
      });
    });
  };
