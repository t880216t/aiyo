export const resolveOpenCodeEnvConfig = (options = {}) => {
  ...
  const configuredOpenCodePort = (() => {
    const raw =
      env.OPENCODE_PORT ||
      env.AIYO_OPENCODE_PORT ||
      env.AIYO_INTERNAL_PORT;
    if (!raw) {
      return null;
    }
  ...
  const configuredOpenCodeHostname = (() => {
    const raw = env.AIYO_OPENCODE_HOSTNAME;
    ...
    if (!trimmed) {
      logger.warn(
        `[config] Ignoring AIYO_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: empty after trimming`,
      );
      return '127.0.0.1';
    }
    return trimmed;
