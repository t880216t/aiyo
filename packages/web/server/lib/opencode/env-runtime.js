...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.AIYO_NODE_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.AIYO_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but AiYo could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
  ...
  const applyOpencodeBinaryFromSettings = async (options = {}) => {
    ...
    try {
      ...
      if (explicitWslPath && explicitWslPath[1] && explicitWslPath[1].trim().length > 0) {
        ...
        console.warn(`Configured settings.opencodeBinary uses WSL, which is no longer supported by AiYo desktop: ${raw}`);
        return null;
      }
      ...
      if (process.platform === 'win32' && (isWslExecutableValue(raw) || isWslExecutableValue(normalized || ''))) {
        ...
        console.warn(`Configured settings.opencodeBinary points to WSL, which is no longer supported by AiYo desktop: ${raw}`);
        return null;
      }
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.AIYO_GIT_BINARY]
