...
## Purpose
This module contains the AiYo message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- AiYo still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `aiyo:session-status`, `aiyo:session-activity`, `aiyo:notification`, and `aiyo:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
