...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.AIYO_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `AiYo refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or AIYO_UI_PASSWORD before exposing it over LAN, '
  + 'or set AIYO_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
