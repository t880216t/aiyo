...
export const normalizeCustomOpenAIBaseURL = (value) => {
  ...
  const isDesktop = (process.env.AIYO_RUNTIME || '').trim().toLowerCase() === 'desktop';
  const envFlagRaw = process.env.AIYO_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  if (!allowRemote && !isAllowedLocalHost(parsed.hostname)) {
    return {
      error: 'Remote custom server URLs are disabled. Set AIYO_ALLOW_REMOTE_OPENAI_COMPAT_URLS=true to allow this host.',
    };
  }
