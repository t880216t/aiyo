...
export const createPushRuntime = (deps) => {
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:aiyo@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set AIYO_VAPID_SUBJECT or enable push once from a real origin.');
    }
