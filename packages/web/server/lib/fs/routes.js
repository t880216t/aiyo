...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.AIYO_FS_EXEC_TIMEOUT_MS);
...
const createGitReadCacheTtlMs = () => {
  const raw = Number(process.env.AIYO_GIT_READ_CACHE_TTL_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 30 * 1000;
};

const createGitCheckIgnoreTimeoutMs = () => {
  const raw = Number(process.env.AIYO_GIT_CHECK_IGNORE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 2500;
};
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, aiyoUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, aiyoUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(aiyoUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, aiyoUserConfigRoot }) => {
...
const resolveReadPathFromContext = async ({ req, targetPath, scope, resolveProjectDirectory, path, os, fsPromises, normalizeDirectoryPath, aiyoUserConfigRoot }) => {
