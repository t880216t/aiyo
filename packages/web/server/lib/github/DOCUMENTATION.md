...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for AiYo.
...
- Auth storage: `~/.config/aiyo/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `AIYO_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `AIYO_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `aiyo.github-pr-status`.
