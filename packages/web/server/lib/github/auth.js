...
const STORAGE_DIR = AIYO_DATA_DIR;
const STORAGE_FILE = path.join(STORAGE_DIR, 'github-auth.json');
const SETTINGS_FILE = path.join(AIYO_DATA_DIR, 'settings.json');
...
function writeJsonFile(payload) {
  ...
  // Atomic write so multiple AiYo instances can safely share the same file.
...
export function getGitHubClientId() {
  const raw = process.env.AIYO_GITHUB_CLIENT_ID;
...
export function getGitHubScopes() {
  const raw = process.env.AIYO_GITHUB_SCOPES;
