...
const RATE_LIMIT_MAX_ATTEMPTS = Number(process.env.AIYO_RATE_LIMIT_MAX_ATTEMPTS) || 10;
const RATE_LIMIT_LOCKOUT_MS = 15 * 60 * 1000;
const RATE_LIMIT_CLEANUP_MS = 60 * 60 * 1000;
const RATE_LIMIT_NO_IP_MAX_ATTEMPTS = Number(process.env.AIYO_RATE_LIMIT_NO_IP_MAX_ATTEMPTS) || 3;
...
const JWT_SECRET_FILE = path.join(AIYO_DATA_DIR, 'jwt-secret');

function getOrCreateJwtSecret() {
  const envSecret = process.env.AIYO_JWT_SECRET;
  if (envSecret) {
  ...
  try {
    fs.mkdirSync(AIYO_DATA_DIR, { recursive: true });
...
function persistJwtSecret(secret) {
  if (process.env.AIYO_JWT_SECRET) {
    const error = new Error('Global sign-out is unavailable while AIYO_JWT_SECRET is set');
