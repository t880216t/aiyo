...
const pwaDevEnabled = process.env.AIYO_DISABLE_PWA_DEV !== '1';
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@aiyo/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.AIYO_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.AIYO_PORT || 3001}`,
