...
description: Open a running dev server inside AiYo.
---

# Preview & Dev Servers

When you start a dev server, AiYo can open it right inside the app instead of a separate browser tab — so you can see your site next to the chat, capture its console, and point at elements to ask about them.

## Open a preview

AiYo watches terminal output for a local address (the `Local:` line that tools like Vite, Next.js, or Astro print). When it spots one:
