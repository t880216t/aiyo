---
title: Reverse proxy
description: Skonfiguruj AiYo poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz AiYo za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że AiYo działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli AiYo już kompresuje odpowiedzi.
...
- AiYo dostępny bezpośrednio w LAN
...
- AiYo kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez AiYo, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
