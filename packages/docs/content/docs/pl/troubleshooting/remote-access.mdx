---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do AiYo z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do AiYo z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.
...
- potwierdź, że serwer działa, za pomocą `aiyo status`

## Link tunelu nie działa

- uruchom `aiyo tunnel status --all`
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, AiYo wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — AiYo nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś AiYo za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse Proxy](/pl/reverse-proxy/).
