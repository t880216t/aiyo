...
description: Налаштуйте AiYo коректно за Nginx, Nginx Proxy Manager або іншим зворотним проксі.
---

# Зворотний проксі

Скористайтеся цією сторінкою, якщо запускаєте AiYo за Nginx, Nginx Proxy Manager, Caddy, Cloudflare або іншим зворотним проксі.

## Перш ніж проксувати

1. Спершу переконайтеся, що AiYo працює напряму.
...
- Вимкніть gzip на проксі, якщо AiYo уже стискає відповіді.
...
- AiYo стискає HTTP-відповіді через gzip (поріг 1 КБ).
...
SSE-маршрути потоків виключені зі стиснення в AiYo, але CDN усе одно може їх буферизувати. Перевірте документацію свого CDN щодо вимкнення буферизації на SSE-шляхах.
