...
Before AiYo can do anything, it needs at least one AI provider connected. This page covers connecting a provider, choosing a model, and tailoring agents.
...
3. Sign in one of two ways, depending on the provider:
   ...
   - **Sign-in (device flow)** — AiYo shows a link and a short code. Open the link, enter the code, and approve. AiYo finishes connecting on its own.
...
Provider sign-ins are stored by OpenCode, not AiYo, so they're shared with the OpenCode CLI. If you set the same provider in more than one place, the most specific setting wins: a per-project setting overrides your personal one.
