...
A project is a folder on your computer that AiYo keeps track of — usually one codebase. Switching projects switches the folder the agent works in, along with that project's sessions and settings.
...
Point it at a folder and AiYo remembers it. The name comes from the folder; you can change it later.
...
Open **Settings → Projects** to give a project a custom name, color, or icon. AiYo tries to find an icon automatically — see [Project icons](/project-icons/).

> In VS Code, AiYo always uses the folder you have open as the only project, so there's nothing to add or switch. The Projects settings page is hidden there.
