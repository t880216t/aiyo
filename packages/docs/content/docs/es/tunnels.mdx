---
title: Túneles
description: Expón AiYo de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu AiYo, para que puedas acceder a él desde tu teléfono u otra red. Usa `aiyo tunnel` para crear uno para una instancia en marcha.

## Requisitos previos

AiYo inicia el CLI del proveedor de túnel en tu máquina. Instala primero el proveedor que quieras usar:
...
Si omites este paso, `aiyo tunnel start` puede iniciar automáticamente un servidor CLI. Al hacerlo, puedes pasar opciones del servidor como `--port`, `--host`, `--lan`, `--ui-password` y `--api-only`.
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a AiYo desde cualquier lugar.

Por defecto, AiYo imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- un único túnel activo por instancia de AiYo (puerto)
...
- [PWA y acceso móvil](/es/mobile/) — accede a AiYo desde tu teléfono
