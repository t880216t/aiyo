...
Rendering and hosting still happen in `aiyo-website` (`apps/docs`).
...
- creates `aiyo-docs-source-<sha>.tar.gz`
...
to `aiyo-website` so it re-syncs and redeploys the docs. This fires on
...
- `AIYO_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `t880216t/aiyo-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `aiyo` (`main` on a
  push, the tag on a release)
- `archive_name`

`aiyo-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`aiyo` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `AIYO_REPO_TOKEN`
