...
![VS Code Extension](https://github.com/t880216t/aiyo/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/t880216t/aiyo) with even more features.**
...
Plus everything from the shared AiYo UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `AiYo: Focus Chat` | Focus the chat panel |
| `AiYo: New Session` | Start a new chat session |
| `AiYo: Open Sidebar` | Open the AiYo sidebar |
| `AiYo: Open Agent Manager` | Launch parallel multi-model runs |
| `AiYo: Open Session in Editor` | Open current or new session in an editor tab |
| `AiYo: Settings` | Open extension settings |
| `AiYo: Restart API Connection` | Restart the OpenCode API process |
| `AiYo: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **AiYo** submenu:
...
|---------|---------|-------------|
| `aiyo.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `aiyo.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `AIYO_VSCODE_BIN=cursor bun run vscode:dev`
- `AIYO_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/aiyo-*.vsix`
