...
describe('VS Code webview request body transport', () => {
  test('preserves body from SDK-style Request objects', async () => {
    const request = new Request('https://aiyo.local/api/session/abc/prompt_async', {
      method: 'POST',
  ...
  test('preserves string, URLSearchParams, Blob, ArrayBuffer, typed array, and FormData bodies', async () => {
    ...
    for (const entry of cases) {
      const encoded = await extractBodyBase64('https://aiyo.local/api/test', entry.init, 'POST');
    ...
    const encodedForm = await extractBodyBase64('https://aiyo.local/api/upload', { body: form }, 'POST');
  ...
  test('extracts text for direct session message bridge bodies', async () => {
    expect(await extractBodyText('https://aiyo.local/api/session/abc/message', { body: new URLSearchParams({ q: 'hello' }) }, 'POST'))
      .toBe('q=hello');
  });
});
