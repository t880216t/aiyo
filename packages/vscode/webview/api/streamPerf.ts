...
declare global {
  interface Window {
    __aiyoVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__aiyoVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__aiyoVsCodeStreamPerfState__ = {
  ...
  return window.__aiyoVsCodeStreamPerfState__;
};
