...
function isWindowsOpenCodeDesktopAppPath(candidate: string): boolean {
  ...
  const normalizeWindowsPath = (value: string) => path.resolve(value).replaceAll('\\', '/').toLowerCase();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromAiYo && isExecutable(sharedFromAiYo) && !isKnownOpenCodeDesktopAppPath(sharedFromAiYo)) {
