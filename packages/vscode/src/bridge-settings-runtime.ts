...
const AIYO_SHARED_SETTINGS_PATH = path.join(os.homedir(), '.config', 'aiyo', 'settings.json');
const AIYO_MAGIC_PROMPTS_PATH = path.join(os.homedir(), '.config', 'aiyo', 'magic-prompts.json');
...
const readSharedSettingsFromDisk = (): Record<string, unknown> => {
  try {
    const raw = fs.readFileSync(AIYO_SHARED_SETTINGS_PATH, 'utf8');
...
const writeSharedSettingsToDisk = async (changes: Record<string, unknown>): Promise<void> => {
  try {
    await fs.promises.mkdir(path.dirname(AIYO_SHARED_SETTINGS_PATH), { recursive: true });
    ...
    const tmp = `${AIYO_SHARED_SETTINGS_PATH}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    await fs.promises.writeFile(tmp, JSON.stringify(next, null, 2), 'utf8');
    await fs.promises.rename(tmp, AIYO_SHARED_SETTINGS_PATH);
  } catch {
    // ignore
  }
};
...
const readMagicPromptFile = (): { version: number; overrides: Record<string, string> } => {
  try {
    const raw = fs.readFileSync(AIYO_MAGIC_PROMPTS_PATH, 'utf8');
...
const writeMagicPromptFile = async (state: { version: number; overrides: Record<string, string> }): Promise<void> => {
  await fs.promises.mkdir(path.dirname(AIYO_MAGIC_PROMPTS_PATH), { recursive: true });
  await fs.promises.writeFile(AIYO_MAGIC_PROMPTS_PATH, JSON.stringify(state, null, 2), 'utf8');
};
