...
const SETTINGS_KEY = 'aiyo.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('AiYo');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('aiyo.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[AiYo] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    const attempted = context.globalState.get<boolean>('aiyo.sidebarAutoMoveAttempted') || false;
    if (attempted) return;
    await context.globalState.update('aiyo.sidebarAutoMoveAttempted', true);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.aiyo');
      } catch (e) {
        outputChannel?.appendLine(`[AiYo] workbench.view.extension.aiyo failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('aiyo.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[AiYo] aiyo.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('AiYo: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[AiYo] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('AiYo: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('aiyo.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[AiYo] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('AiYo: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.focusChat', async () => {
      await vscode.commands.executeCommand('aiyo.chatView.focus');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('AiYo: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('AiYo: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('AiYo: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('AiYo [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('AiYo [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('AiYo: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('AiYo: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('AiYo [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('AiYo [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('AiYo [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('AiYo: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`AiYo: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('aiyo.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('aiyo');
      ...
      const lines = [
        ...
        `AiYo version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('aiyo').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
