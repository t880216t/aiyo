...
describe('VS Code config bridge plugin parity', () => {
  test('creates, lists, updates, and deletes project plugin entries', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aiyo-vscode-plugins-'));
    tempRoots.push(root);
  ...
  test('creates and reads project plugin files', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aiyo-vscode-plugin-files-'));
    tempRoots.push(root);
    const ctx = createCtx(root);
  ...
  test('updates and deletes user plugin entries from OPENCODE_CONFIG source', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aiyo-vscode-custom-config-'));
    tempRoots.push(root);
  ...
  test('writes user plugin files next to OPENCODE_CONFIG', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aiyo-vscode-custom-files-'));
    tempRoots.push(root);
    const configDir = path.join(root, 'custom-config');
    const configPath = path.join(configDir, 'opencode.json');
    fs.mkdirSync(configDir, { recursive: true });
  ...
  test('reports plugin mutation success when restart fails after writing config', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aiyo-vscode-plugin-restart-'));
    tempRoots.push(root);
