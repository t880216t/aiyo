...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches AiYoLogo.tsx:
...
<body>
  <!-- Initial loading screen with simplified AiYo logo -->
  ...
  <script>
    ...
    window.__AIYO_HOME__ = "${workspaceFolder.replace(/\\/g, '\\\\')}";
    
    function getBootstrapMessages() {
      var locale = 'en';
      try {
        var rawLocale = window.localStorage.getItem('aiyo.i18n.v1');
        if (rawLocale) {
  ...
  <script type="module">
    ...
    if (!devServerUrl) {
      ...
      const getDevMessages = () => {
        try {
          const rawLocale = window.localStorage.getItem('aiyo.i18n.v1');
          if (rawLocale) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[AiYo] VS Code webview dev bundle unavailable, retrying...', error);
