# xspace-browser Browser Tools Reference

Complete parameter schemas and usage examples for all xspace-browser MCP tools exposed via `xspace-browser-mcp-bridge`.
...
Get the configured proxy profiles and the currently active proxy.

**Parameters:** none

**Returns:** Array of proxy profiles — each with `profileID`, `name`, `type`, `server` address, `group`, and `active` (whether it is the currently applied proxy).

---

### `set_proxy`

Apply a proxy profile in the connected browser. The change takes effect immediately for all subsequent requests.

| Parameter | Type | Required | Description |
|---|---|---|---|
| `profileID` | string | yes | The `profileID` of the proxy to activate (obtained from `get_proxy_list`) |

**Workflow:** always call `get_proxy_list` first to obtain the `profileID` — never guess it.
...
List the xspace-browser instances connected to this bridge.

**Parameters:** none

**Returns:** Array of `{ name, instanceId }` plus which one (if any) is selected for this session.

---

### `select_browser`

When multiple browsers are connected, choose which one this session's tool calls target. Required before any tool call once 2+ browsers are connected.

| Parameter | Type | Required | Description |
|---|---|---|---|
| `name` | string | yes | The name of the browser instance to target (from `list_browsers`) |
...
└── NO ──▶ What kind of task?
              ...
              ├── Switch network proxy ──▶ get_proxy_list ──▶ set_proxy(profileID)
