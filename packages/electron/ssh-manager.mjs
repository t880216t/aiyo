...
export class ElectronSshManager {
  ...
  async spawnMasterProcess(parsed, controlPath, askpassPath, sshPassword) {
    const child = spawn('ssh', buildSshArgs(parsed, [
      ...
      env: {
        ...
        DISPLAY: '1',
        ...(sshPassword ? { AIYO_SSH_ASKPASS_VALUE: sshPassword.trim() } : {}),
      },
    });
    return child;
  }
  ...
  async currentRemoteAiYoVersion(parsed, controlPath) {
    try {
      const output = await runRemoteCommand(parsed, controlPath, 'aiyo --version 2>/dev/null || true');
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    ...
    const output = await runRemoteCommand(parsed, controlPath, `${envPrefix} aiyo serve --hostname 127.0.0.1 --port ${desiredPort}`);
