...
const SSH_STATUS_EVENT = 'aiyo:ssh-instance-status';
...
if [[ -n "$AIYO_SSH_ASKPASS_VALUE" ]]; then
  if [[ "$PROMPT" == *"assword"* || "$PROMPT" == *"passphrase"* ]]; then
    printf '%s\\n' "$AIYO_SSH_ASKPASS_VALUE"
    exit 0
  fi
fi
...
const windowsAskpassScriptContent = () => "$value = [Environment]::GetEnvironmentVariable('AIYO_SSH_ASKPASS_VALUE')\nif ($null -ne $value) { [Console]::Out.WriteLine($value) }\n";
const windowsAskpassWrapperContent = () => '@echo off\r\n"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%~dp0askpass.ps1"\r\n';
...
const sanitizeProcessDiagnostic = (value, secret = '') => {
  ...
  return sanitized.length > MAX_PROCESS_ERROR_CHARS
    ? `${sanitized.slice(0, MAX_PROCESS_ERROR_CHARS - 3)}...`
    : sanitized;
};
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded AiYo health');
};
...
export class ElectronSshManager {
  ...
  authEnvironment(parsed) {
    ...
    return {
      ...
      DISPLAY: '1',
      ...(auth.sshPassword ? { AIYO_SSH_ASKPASS_VALUE: auth.sshPassword } : {}),
    };
  }

  independentConnectionArgs() {
    return ['-o', 'ControlMaster=no', '-o', 'ControlPath=none', '-o', 'StrictHostKeyChecking=accept-new'];
  }

  trackSshProcess(child, parsed) {
    const diagnostics = { stderr: '', error: null, parsed };
    childProcessDiagnostics.set(child, diagnostics);
    this.sshAuth.get(parsed)?.children.add(child);
    child.stderr?.on('data', (chunk) => {
      diagnostics.stderr = `${diagnostics.stderr}${chunk.toString()}`.slice(-MAX_PROCESS_ERROR_CAPTURE_CHARS);
    });
    child.on('error', (error) => { diagnostics.error = error; });
    child.on('close', () => { this.sshAuth.get(parsed)?.children.delete(child); });
    return child;
  }
  ...
  spawnSsh(parsed, preDestinationArgs, options, remoteCommand = null) {
    return this.trackSshProcess(this.spawnProcess('ssh', buildSshArgs(parsed, preDestinationArgs, remoteCommand), {
      ...options,
  ...
  async runSshOutput(parsed, preDestinationArgs, remoteCommand = null) {
    return await new Promise((resolve, reject) => {
      ...
      child.stderr?.on('data', (chunk) => { stderr = `${stderr}${chunk.toString()}`.slice(-MAX_PROCESS_ERROR_CAPTURE_CHARS); });
      child.on('error', () => reject(new Error(this.processErrorDetail(child, 'Failed to start SSH process'))));
      child.on('close', (code) => resolve({
        code: typeof code === 'number' ? code : -1,
        stdout,
        stderr: sanitizeProcessDiagnostic(stderr, this.sshAuth.get(parsed)?.sshPassword),
      }));
    });
  }
  ...
  async runRemoteCommand(parsed, controlPath, script, timeoutSec = DEFAULT_CONNECTION_TIMEOUT_SEC) {
    ...
    const { code, stdout, stderr } = await this.runSshOutput(parsed, [
      ...connectionArgs, '-o', `ConnectTimeout=${timeoutSec}`, '-T',
    ], `sh -lc ${shellQuote(script)}`);
    if (code !== 0) throw new Error(stderr || stdout || 'Remote command failed');
    return stdout;
  }

  async controlMasterOperation(parsed, controlPath, op) {
    return await this.runSshOutput(parsed, ['-o', 'ControlMaster=no', '-o', `ControlPath=${controlPath}`, '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=3', '-O', op]);
  }

  async isControlMasterAlive(parsed, controlPath) {
    if (!this.usesControlMaster()) return true;
  ...
  async stopControlMasterBestEffort(parsed, controlPath) {
    ...
    try { await this.controlMasterOperation(parsed, controlPath, 'exit'); } catch {}
  }
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      remoteAiyo: {
        mode: instance?.remoteAiyo?.mode === 'external' ? 'external' : 'managed',
        keepRunning: instance?.remoteAiyo?.keepRunning !== false,
        ...(Number.isFinite(instance?.remoteAiyo?.preferredPort) ? { preferredPort: Number(instance.remoteAiyo.preferredPort) } : {}),
        installMethod: ['npm', 'bun', 'download_release', 'upload_bundle'].includes(instance?.remoteAiyo?.installMethod)
          ? instance.remoteAiyo.installMethod
          : 'bun',
        uploadBundleOverSsh: Boolean(instance?.remoteAiyo?.uploadBundleOverSsh),
      },
      localForward: {
      ...
      auth: {
        ...
        ...(this.sanitizeStoredSecret(instance?.auth?.aiyoPassword) ? { aiyoPassword: this.sanitizeStoredSecret(instance.auth.aiyoPassword) } : {}),
      },
      portForwards,
    };
  }
  ...
  configuredAiYoPassword(instance) {
    const secret = instance?.auth?.aiyoPassword;
  ...
  async currentRemoteAiYoVersion(parsed, controlPath) {
    try {
      const output = await this.runRemoteCommand(parsed, controlPath, 'aiyo --version 2>/dev/null || true');
  ...
  async installAiYoManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @aiyo/web@${version}`);
      if (hasNpm) commands.push(`npm install -g @aiyo/web@${version}`);
    ...
    throw lastError || new Error('Failed to install AiYo on remote host');
  }

  async probeRemoteSystemInfo(parsed, controlPath, port, aiyoPassword) {
    const authPayload = aiyoPassword ? JSON.stringify({ password: aiyoPassword }) : '{}';
    const authEnabled = aiyoPassword ? '1' : '0';
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (aiyoPassword && authStatus !== 200) {
          throw new Error(`Remote AiYo requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote AiYo requires UI authentication on /api/system/info; configure AiYo UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote AiYo probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async remoteServerRunning(parsed, controlPath, port, aiyoPassword) {
    try {
      await this.probeRemoteSystemInfo(parsed, controlPath, port, aiyoPassword);
      return true;
    } catch {
      return false;
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'AIYO_RUNTIME=ssh-remote';
    const secret = this.configuredAiYoPassword(instance);
    if (secret) {
      envPrefix += ` AIYO_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await this.runRemoteCommand(parsed, controlPath, `${envPrefix} aiyo serve --hostname 127.0.0.1 --port ${desiredPort}`);
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteAiyo.mode === 'external') {
      if (!instance.remoteAiyo.preferredPort) {
        throw new Error('External mode requires a preferred remote AiYo port');
      }
      const port = instance.remoteAiyo.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external AiYo server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredAiYoPassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote AiYo installation');
    const installedVersion = await this.currentRemoteAiYoVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing AiYo on remote host');
      await this.installAiYoManaged(parsed, controlPath, this.appVersion, instance.remoteAiyo.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote AiYo from ${installedVersion} to ${this.appVersion}`);
      await this.installAiYoManaged(parsed, controlPath, this.appVersion, instance.remoteAiyo.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed AiYo server');
    let remotePort = instance.remoteAiyo.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredAiYoPassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed AiYo server');
      const desiredPort = instance.remoteAiyo.preferredPort || randomPortCandidate(instance.id);
      remotePort = await this.startRemoteServerManaged(parsed, controlPath, instance, desiredPort);
      startedByUs = true;
    }
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredAiYoPassword(instance)))) {
      throw new Error('Managed AiYo server failed to become reachable');
    }
  ...
  async disconnectInternal(id, reportIdle) {
    ...
    if (session) {
      if (session.startedByUs && session.instance.remoteAiyo.mode === 'managed' && !session.instance.remoteAiyo.keepRunning) {
      ...
      for (const child of [session.mainForward, session.master, ...(session.extraForwards || [])].filter(Boolean)) {
        try {
          child.kill('SIGTERM');
        } catch {
        }
      ...
      for (const askpassPath of session.askpassCleanupPaths || [path.join(session.sessionDir, 'askpass.sh')]) {
        try { await fsp.rm(askpassPath, { force: true }); } catch {}
      }
  ...
  async connectBlocking(instance) {
    ...
    const master = this.usesControlMaster() ? await this.spawnMasterProcess(parsed, controlPath) : null;
    if (master) await this.waitForMasterReady(parsed, controlPath, instance.connectionTimeoutSec || DEFAULT_CONNECTION_TIMEOUT_SEC, master);
    ...
    for (const forward of instance.portForwards.filter((item) => item.enabled)) {
      try {
        ...
        if (extraForward) extraForwards.push(extraForward);
