...
import { mintOutsideFileGrant } from '@aiyo/web/server/lib/fs/routes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.AIYO_ELECTRON_DEV === '1' || !app.isPackaged;

const DEEP_LINK_PROTOCOL = 'aiyo';
const UI_PROTOCOL = 'aiyo-ui';
const PACKAGED_APP_USER_MODEL_ID = 'dev.aiyo.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.aiyo.desktop.dev';
...
// ~/Library/Logs/AiYo/ (not ~/Library/Logs/@aiyo/electron/).
app.setName('AiYo');
if (isDev) {
...
// that output never lands in ~/Library/Logs/AiYo/main.log and we
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@aiyo/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@aiyo/electron', version: app.getVersion() };
};
...
const GITHUB_BUG_REPORT_URL = 'https://github.com/t880216t/aiyo/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/t880216t/aiyo/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `AiYo detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/aiyo/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/aiyo/tunnel/status`;
...
const settingsFilePath = () => {
  if (typeof process.env.AIYO_DATA_DIR === 'string' && process.env.AIYO_DATA_DIR.trim()) {
    return path.join(process.env.AIYO_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'aiyo', 'settings.json');
};
...
const shouldUsePackagedUi = () => {
  if (process.env.AIYO_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.AIYO_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
const injectRuntimeConfigIntoHtml = (html) => {
  ...
  const initScript = `<script>if(window.__AIYO_LOCAL_ORIGIN__===undefined){window.__AIYO_LOCAL_ORIGIN__=${JSON.stringify(localOrigin)};}if(window.__AIYO_API_BASE_URL__===undefined){window.__AIYO_API_BASE_URL__=${JSON.stringify(apiBaseUrl)};}if(window.__AIYO_CLIENT_TOKEN__===undefined&&${JSON.stringify(state.clientToken || '')}){window.__AIYO_CLIENT_TOKEN__=${JSON.stringify(state.clientToken || '')};}</script>`;
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToAllWindows('aiyo:open-session', { sessionId, directory });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@aiyo/web/server/lib/opencode/path-utils.js';
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / AIYO_DIST_DIR /
  // AIYO_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.AIYO_HOST = bindHost;
  process.env.AIYO_DESKTOP_LAN_ACCESS_ACTIVE = effectiveLanAccessEnabled ? 'true' : 'false';
  if (lanAccessBlockedByMissingPassword) {
    process.env.AIYO_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
  } else {
    delete process.env.AIYO_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
  }
  process.env.AIYO_DIST_DIR = resolveWebDistDir();
  ...
  process.env.AIYO_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  process.env.AIYO_DESKTOP_NOTIFY = 'true';
  if (desktopUiPassword) {
    process.env.AIYO_UI_PASSWORD = desktopUiPassword;
  } else {
    delete process.env.AIYO_UI_PASSWORD;
  }
  process.env.AIYO_SKIP_API_COMPRESSION = process.env.AIYO_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  const { startWebUiServer } = await import('@aiyo/web/server/index.js');
...
`;
  ...
  const child = spawn('/bin/sh', ['-c', script, 'aiyo-opencode-killer', normalizedPid, normalizedPort, String(OPENCODE_SHUTDOWN_GRACE_MS / 1000)], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="AiYo loading icon">
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('aiyo:emit', { event, detail });
};
...
const setMacVibrancyReady = (browserWindow, ready) => {
  if (process.platform !== 'darwin' || !browserWindow || browserWindow.isDestroyed()) return;
  emitToWindow(browserWindow, 'aiyo:vibrancy-ready', { ready });
};
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'session' && link.value) {
    emitToAllWindows('aiyo:open-session', { sessionId: link.value });
    return;
  }
  if (link.type === 'project' && link.value) {
    emitToAllWindows('aiyo:open-project', { projectPath: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'aiyo:menu-action', action);
  dispatchDomEventToWindow(target, 'aiyo:menu-action', action);
};
...
const dispatchOpenMiniChat = (browserWindow) => {
  ...
  if (target) emitToWindow(target, 'aiyo:open-mini-chat');
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('aiyo:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'aiyo:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url, runtimeConfig = {} }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        ...
        `--aiyo-macos-major=${desktopMacosMajor}`,
        `--aiyo-mac-vibrancy=${useVibrancy ? '1' : '0'}`,
        `--aiyo-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
      ],
  ...
  browserWindow.on('resize', () => {
    if (process.platform === 'darwin') {
      emitToWindow(browserWindow, 'aiyo:window-resized');
    }
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'aiyo:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'aiyo:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.AIYO_HMR_API_PORT || '3901';
  ...
  const envTarget = normalizeHostUrl(process.env.AIYO_SERVER_URL || '');
  const config = readDesktopHostsConfig();
  if (envTarget) {
...
const parseGithubRepo = () => {
  return { owner: 't880216t', repo: 'aiyo' };
};
...
const setupAutoUpdater = () => {
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('aiyo:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const iconToDataUrl = async (iconPath, appName) => {
  ...
  const tempPath = path.join(os.tmpdir(), `aiyo-icon-${safeName}-${Date.now()}.png`);
  try {
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/aiyo/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('aiyo:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires AiYo.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window': {
      ...
      if (sessionId) {
        emitToWindow(state.mainWindow, 'aiyo:open-session', { sessionId, directory });
      } else if (mode === 'draft') {
        emitToWindow(state.mainWindow, 'aiyo:open-draft-session', { directory, projectId });
      }
      return { focused: true };
    }
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About AiYo', click: () => dispatchAction('about') },
        {
...
ipcMain.handle('aiyo:invoke', async (event, command, args) => {
...
ipcMain.handle('aiyo:dialog:open', async (event, options) => {
...
ipcMain.handle('aiyo:file:grant-existing', async (event, filePath) => {
  if (!isLocalSender(event.sender)) {
...
const focusMainWindowWithSession = async (sessionId, directory) => {
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    ...
    state.mainWindow.focus();
    if (sessionId) {
      emitToWindow(state.mainWindow, 'aiyo:open-session', { sessionId, directory: directory || '' });
    }
    return;
  }
  if (sessionId) pendingDeepLinks.push({ type: 'session', value: sessionId });
...
const dispatchTrayAction = async (action) => {
  ...
  if (action.type === 'respond-permission') {
    ...
    emitToWindow(target, 'aiyo:tray-action', action);
    return;
  }
  ...
  if (action.type === 'focus-session') {
    ...
    if (surface && surface.__ocMiniChat === true && action.sessionId) {
      ...
      surface.show();
      surface.focus();
      emitToWindow(surface, 'aiyo:open-session', {
  ...
  if (action.type === 'new-session') {
    emitToWindow(target, 'aiyo:open-draft-session', { directory: '', projectId: '' });
  }
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('aiyo:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {
