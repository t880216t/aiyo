...
Electron desktop runtime for AiYo on macOS and Windows.

This package owns the native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web UI and AiYo server logic still live in `packages/web` and shared React UI lives in `packages/ui`.

## How It Runs

Desktop starts the AiYo web server in the same Electron main process. There is no separate sidecar subprocess for the AiYo server.

`main.mjs` imports `@aiyo/web/server/index.js` and calls `startWebUiServer()`. The Electron window then loads the UI from the local server in development, or from packaged `resources/web-dist` assets in packaged builds.

The preload bridge exposes desktop-only APIs to the web UI through `window.__AIYO_DESKTOP__`. Privileged commands are checked in `main.mjs`, not only in the UI.
...
| Variable | Use |
|----------|-----|
| `AIYO_ELECTRON_DEV=1` | Marks the runtime as desktop development mode |
| `AIYO_ELECTRON_USE_BUNDLED_UI=1` | Uses staged web assets instead of the HMR dev server |
| `AIYO_HMR_UI_PORT` | Preferred Vite UI port for desktop dev, default `5173` |
| `AIYO_HMR_API_PORT` | Preferred API port for desktop dev, default `3901` |
| `AIYO_RUNTIME=desktop` | Set by Electron before starting the web server |
| `AIYO_DESKTOP_NOTIFY=true` | Enables desktop notification flow in the web server |
| `AIYO_SKIP_API_COMPRESSION=true` | Defaulted by Desktop to reduce local CPU overhead |
...
2. Add the real command handling in `main.mjs` under `aiyo:invoke`.
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `AiYo` app name.

Development builds use a separate user data directory named `AiYo Dev`, so dev state does not overwrite normal packaged app state.
...
- Keep `@aiyo/web`, `bun-pty`, `node-pty`, and native modules external in `bundle-main.mjs`; bundling them can break Electron startup.
