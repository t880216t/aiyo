...
const ensureWindowsNodeAddonApiForNodePty = async (rebuildRootPath) => {
  ...
  const tempNodeAddonApiDir = path.join(repoRoot, 'node_modules', '.aiyo-node-addon-api-7.1.1');
  const exportedTempNodeAddonApiDir = path.join(rebuildRootPath, 'node_modules', '.aiyo-node-addon-api-7.1.1');
