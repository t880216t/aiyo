...
const preferredHmrUiPort = Number(process.env.AIYO_HMR_UI_PORT || '5173');
const preferredHmrApiPort = Number(process.env.AIYO_HMR_API_PORT || '3901');
...
function spawnProcess(command, args, options = {}) {
  ...
  return spawn(spawnCommand, spawnArgs, {
    cwd: repoRoot,
    env: { ...process.env, AIYO_ELECTRON_DEV: '1' },
    stdio: 'inherit',
...
async function main() {
  const useBundledUi = process.env.AIYO_ELECTRON_USE_BUNDLED_UI === '1';
  let devServer = null;
  let hmrApiPort = '';
  let hmrUiPort = '';
  ...
  if (useBundledUi) {
    ...
    devServer = spawnProcess('node', ['./scripts/dev-web-hmr.mjs'], {
      env: {
        ...process.env,
        AIYO_ELECTRON_DEV: '1',
        AIYO_HMR_UI_PORT: hmrUiPort,
        AIYO_HMR_API_PORT: hmrApiPort,
        AIYO_DISABLE_PWA_DEV: '1',
      },
    });
  }
  ...
  const electron = spawnProcess('npx', ['electron', './main.mjs'], {
    ...
    env: {
      ...process.env,
      AIYO_ELECTRON_DEV: '1',
      ...(useBundledUi ? { AIYO_ELECTRON_USE_BUNDLED_UI: '1' } : {}),
      AIYO_HMR_UI_PORT: hmrUiPort,
      AIYO_HMR_API_PORT: hmrApiPort,
      AIYO_DISABLE_PWA_DEV: '1',
    },
