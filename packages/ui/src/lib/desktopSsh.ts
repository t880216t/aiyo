...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    aiyoPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteAiYo)
    ? value.remoteAiYo
    : isRecord(value.remote_aiyo)
      ? value.remote_aiyo
      : {};
  ...
  const aiyoPassword = parseStoredSecret(authRaw.aiyoPassword || authRaw.aiyo_password);
...
export const listenDesktopSshStatus = async (
  ...
  const desktop = (window as unknown as { __AIYO_DESKTOP__?: DesktopBridgeGlobal }).__AIYO_DESKTOP__;
  const listen = desktop?.listen;
  if (typeof listen !== 'function') {
    return async () => {};
  }

  const unlisten = await listen('aiyo:ssh-instance-status', (event) => {
