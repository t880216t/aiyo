...
declare global {
  interface Window {
    __AIYO_STARTUP_TRACE__?: StartupTraceEvent[];
    __AIYO_STARTUP_TRACE_START__?: number;
  }
}
...
const enabled = () => {
  ...
  try {
    ...
    return params.get('startupTrace') === '1' || window.localStorage?.getItem('AIYO_STARTUP_TRACE') === '1';
  } catch {
    return false;
  }
};
...
export const markStartupTrace = (name: string, data?: Record<string, unknown>) => {
  ...
  window.__AIYO_STARTUP_TRACE_START__ ??= now;
  window.__AIYO_STARTUP_TRACE__ ??= [];
  window.__AIYO_STARTUP_TRACE__.push({
    t: Math.round(now - window.__AIYO_STARTUP_TRACE_START__),
    name,
    ...(data ? { data } : {}),
  });
  if (window.__AIYO_STARTUP_TRACE__.length > MAX_STARTUP_TRACE_EVENTS) {
    window.__AIYO_STARTUP_TRACE__.splice(
      0,
      window.__AIYO_STARTUP_TRACE__.length - MAX_STARTUP_TRACE_EVENTS,
    );
  }
};

export const getStartupTraceSummary = () => {
  const trace = typeof window !== 'undefined' ? window.__AIYO_STARTUP_TRACE__ ?? [] : [];
...
if (typeof window !== 'undefined') {
  (window as typeof window & { __AIYO_STARTUP_TRACE_SUMMARY__?: typeof getStartupTraceSummary })
    .__AIYO_STARTUP_TRACE_SUMMARY__ = getStartupTraceSummary;
}
