...
type AiYoEvent = ScheduledTaskRanEvent;
type Listener = (event: AiYoEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'aiyo:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'aiyo:heartbeat') {
    return;
  }

  if (envelope.type !== 'aiyo:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource(getRuntimeUrlResolver().sse('/api/aiyo/events'));
  source.onopen = () => {
    resetHeartbeatTimer();
  };
...
export const subscribeAiYoEvents = (listener: Listener): (() => void) => {
