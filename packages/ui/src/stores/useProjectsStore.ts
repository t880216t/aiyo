...
const getLocalRuntimeOrigin = (): string => {
  ...
  const value = (window as typeof window & { __AIYO_LOCAL_ORIGIN__?: string }).__AIYO_LOCAL_ORIGIN__;
...
const createVSCodeWorkspaceProjects = (
  ...
  if (streamDebugEnabled()) {
    console.log('[AiYo][VSCode][projects] Using workspace projects', projects);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('aiyo:settings-synced', (event: Event) => {
