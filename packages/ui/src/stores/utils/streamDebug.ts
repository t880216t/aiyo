...
export const sessionStatusDebugEnabled = (): boolean => {
    if (typeof window === 'undefined') return false;
    try {
        return window.localStorage.getItem('aiyo_session_status_debug') === '1';
    } catch {
        return false;
    }
};

const STREAM_PERF_STORAGE_KEY = 'aiyo_stream_perf';
...
declare global {
    interface Window {
        __aiyoStreamPerfState?: StreamPerfState;
        __aiyoVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__aiyoStreamPerfState) {
        const startedAt = Date.now();
        window.__aiyoStreamPerfState = {
    ...
    return window.__aiyoStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__aiyoStreamPerfState;
        delete window.__aiyoVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__aiyoVsCodeStreamPerfState) {
        window.__aiyoVsCodeStreamPerfState = {
            ...window.__aiyoVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__aiyoStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__aiyoVsCodeStreamPerfState;
    if (!streamPerfEnabled() || !state) {
        return {
            enabled: false,
            startedAt: null,
