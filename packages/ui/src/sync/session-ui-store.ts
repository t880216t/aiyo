...
export type SessionUIState = {
  ...
  initializeNewAiYoSession: (sessionId: string, agents: unknown[]) => void
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsAiYoCreated: (sessionId) =>
    set((s) => {
  ...
  isAiYoCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  sendMessage: async (
    ...
    if (!options?.sessionId && draft?.open) {
      ...
      get().initializeNewAiYoSession(created.id, configState.agents ?? [])
