...
function App({ apis }: AppProps) {
  React.useEffect(() => {
    ...
    if (startupTraceEnabled()) {
      console.info('[startup-trace] enabled. Run console.table(window.__AIYO_STARTUP_TRACE__) after startup.');
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'aiyo:embedded-visibility') {
        return;
      }
    ...
    const scopedWindow = window as unknown as {
      __aiyoSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__aiyoSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__aiyoSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__aiyoSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:open-session', handler as EventListener);
    return () => window.removeEventListener('aiyo:open-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:open-mini-chat', onOpenMiniChat);
    return () => window.removeEventListener('aiyo:open-mini-chat', onOpenMiniChat);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:open-draft-session', handler as EventListener);
    return () => window.removeEventListener('aiyo:open-draft-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:open-project', handler as EventListener);
    return () => window.removeEventListener('aiyo:open-project', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __aiyoAppReady?: boolean }).__aiyoAppReady = true;
    window.dispatchEvent(new Event('aiyo:app-ready'));
  ...
  // The Rust backend sets window.__AIYO_DESKTOP_BOOT_OUTCOME__ once the
