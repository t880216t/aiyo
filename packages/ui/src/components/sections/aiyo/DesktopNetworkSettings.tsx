...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.aiyo.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.aiyo.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.aiyo.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.aiyo.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.aiyo.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.aiyo.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.aiyo.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <div className="mb-8">
      <div className="mb-1 px-1">
        <h3 className="typography-ui-header font-medium text-foreground">{t('settings.aiyo.desktopNetwork.title')}</h3>
      </div>
      ...
      <section className="space-y-2 px-2 pb-2 pt-0">
        {launchAtLoginSupported ? (
          ...
          >
            <Checkbox
              ...
              ariaLabel={t('settings.aiyo.desktopNetwork.field.launchAtLoginAria')}
              disabled={isSavingLaunchAtLogin}
            />
            <div className="min-w-0 flex-1">
              <div className="typography-ui-label text-foreground">{t('settings.aiyo.desktopNetwork.field.launchAtLogin')}</div>
              <div className="typography-micro text-muted-foreground/70">
                {t('settings.aiyo.desktopNetwork.field.launchAtLoginDescription')}
              </div>
            </div>
          </div>
        ) : null}
        ...
        <div data-settings-item="sessions.desktop-ui-password" className="space-y-1 py-1.5">
          <label className="typography-ui-label text-foreground" htmlFor="desktop-ui-password">
            {t('settings.aiyo.desktopPassword.field.password')}
          </label>
          <Input
            ...
            placeholder={t('settings.aiyo.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
            aria-invalid={lanRequiresPassword}
          />
          <div className="typography-micro text-muted-foreground/70">
            {t('settings.aiyo.desktopPassword.field.passwordDescription')}
          </div>
        </div>
        ...
        >
          <Checkbox
            ...
            ariaLabel={t('settings.aiyo.desktopNetwork.field.allowLanAccessAria')}
            disabled={isLoading || isSaving}
          />
          <div className="min-w-0 flex-1">
            <div className="typography-ui-label text-foreground">{t('settings.aiyo.desktopNetwork.field.allowLanAccess')}</div>
            <div className="typography-micro text-muted-foreground/70">
              {t('settings.aiyo.desktopNetwork.field.allowLanAccessDescription')}
            </div>
            <div className="typography-micro text-[var(--status-warning)]/85">
              {t('settings.aiyo.desktopNetwork.field.warning')}
            </div>
            {lanRequiresPassword || lanBlockedByMissingPassword ? (
              <div className="typography-micro text-[var(--status-warning)]/85">
                {t('settings.aiyo.desktopNetwork.field.passwordRequiredWarning')}
              </div>
            ) : null}
          </div>
        </div>
        ...
        {lanUrl ? (
          <div className="px-2 typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.aiyo.desktopNetwork.hint.openAfterRestart')
              : t('settings.aiyo.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.aiyo.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </section>
    </div>
