...
export const PasskeySettings: React.FC = () => {
  ...
  const loadPasskeys = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.aiyo.passkeys.toast.loadFailed');
  ...
  const handleRegisterPasskey = React.useCallback(async () => {
    if (!status.enabled) {
      const message = t('settings.aiyo.passkeys.toast.enableUiPasswordFirst');
      setErrorMessage(message);
    ...
    try {
      ...
      await loadPasskeys();
      toast.success(t('settings.aiyo.passkeys.toast.added'));
    } catch (error) {
      if (isPasskeyCeremonyAbort(error)) {
        toast.message(t('settings.aiyo.passkeys.toast.setupCanceled'));
        return;
      }

      const message = error instanceof Error ? error.message : t('settings.aiyo.passkeys.toast.addFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setIsRegistering(false);
  ...
  const handleRevokePasskey = React.useCallback(async (id: string) => {
    ...
    try {
      ...
      setStatus(await fetchPasskeyStatus());
      await loadPasskeys();
      toast.success(t('settings.aiyo.passkeys.toast.removed'));
    } catch (error) {
      const message = error instanceof Error ? error.message : t('settings.aiyo.passkeys.toast.removeFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setRevokingId(null);
  ...
  const handleResetAllAuth = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.aiyo.passkeys.toast.clearAuthFailed');
      setErrorMessage(message);
      toast.error(message);
  ...
  return (
    <div className="mb-8">
      <div className="mb-1 px-1">
        <h3 className="typography-ui-header font-medium text-foreground">{t('settings.aiyo.passkeys.title')}</h3>
      </div>

      <section className="px-2 pb-2 pt-0 space-y-2">
        <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
          <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
            <span className="typography-ui-label text-foreground">{t('settings.aiyo.passkeys.field.currentDevice')}</span>
          </div>
          <div className="flex items-center gap-2 sm:w-fit">
            ...
            >
              {isRegistering ? t('settings.aiyo.passkeys.actions.cancelSetup') : t('settings.aiyo.passkeys.actions.add')}
            </Button>
            <Button
              type="button"
              variant="ghost"
            ...
            >
              {isResetting ? t('settings.aiyo.passkeys.actions.signingOut') : t('settings.aiyo.passkeys.actions.signOutEverywhere')}
            </Button>
          </div>
        </div>
        ...
        {!status.enabled && (
          <p className="typography-meta text-muted-foreground">
            {t('settings.aiyo.passkeys.state.uiPasswordRequired')}
          </p>
        )}
        ...
        {isLoading ? (
          <p className="typography-meta text-muted-foreground">{t('settings.aiyo.passkeys.state.loading')}</p>
        ) : passkeys.length === 0 ? (
          <p className="typography-meta text-muted-foreground">{t('settings.aiyo.passkeys.state.noneSaved')}</p>
        ) : (
              ...
              <div key={passkey.id} className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
                ...
                <div className="flex min-w-0 flex-1 items-center justify-between gap-3">
                  <span className="typography-meta text-muted-foreground truncate">
                    {passkey.lastUsedAt
                      ? t('settings.aiyo.passkeys.item.lastUsed', {
                          time: formatTimestamp(passkey.lastUsedAt, t('settings.aiyo.passkeys.time.neverUsed'), timeFormatPreference),
                        })
                      : t('settings.aiyo.passkeys.item.added', {
                          time: formatTimestamp(passkey.createdAt, t('settings.aiyo.passkeys.time.neverUsed'), timeFormatPreference),
                        })}
                  </span>
                  <Button
                    type="button"
                    ...
                    className="!font-normal text-muted-foreground hover:text-foreground"
                  >
                    {revokingId === passkey.id ? t('settings.aiyo.passkeys.actions.removing') : t('settings.common.actions.delete')}
                  </Button>
                </div>
              </div>
            ))}
