...
const GITHUB_URL = 'https://github.com/t880216t/aiyo';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [aiYoVersion, setAiYoVersion] = React.useState<string | null>(null);
  ...
  const currentVersion = aiYoVersion || updateStore.info?.currentVersion || 'unknown';

  React.useEffect(() => {
    let cancelled = false;

    const loadAiYoVersion = async () => {
      try {
        ...
        const data = await response.json().catch(() => null) as { aiyoVersion?: unknown } | null;
        const version = typeof data?.aiyoVersion === 'string' && data.aiyoVersion.trim().length > 0
          ? data.aiyoVersion.trim()
          : null;
        if (!cancelled) setAiYoVersion(version);
      } catch {
        if (!cancelled) setAiYoVersion(null);
      }
    };

    void loadAiYoVersion();
    ...
    if (updateStore.checking) {
      ...
      const timer = setTimeout(() => {
        ...
        if (didInitiateCheck.current && !updateStore.available && !updateStore.error) {
          toast.success(t('settings.aiyo.about.toast.latestVersion'));
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          <AiYoLogo width={72} height={72} />
          <h2 className="mt-4 typography-ui-header font-semibold text-foreground">AiYo</h2>
          <div className="mt-2 space-y-1 typography-ui text-muted-foreground">
            <p>{t('aboutDialog.aiYoVersionLabel', { version: currentVersion })}</p>
            <p>{t('aboutDialog.openCodeVersionLabel', { version: openCodeVersion || t('settings.aiyo.about.state.unknown') })}</p>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          {!updateStore.available && !updateStore.error && (
            ...
            >
              ...
              {isChecking ? t('settings.aiyo.about.state.checking') : t('settings.aiyo.about.actions.checkForUpdates')}
            </Button>
          )}
          ...
          {!isChecking && updateStore.available && (
            ...
            >
              ...
              {t('settings.aiyo.about.actions.updateToVersion', { version: updateStore.info?.version || '' })}
            </Button>
          )}
        </div>
      ...
      <div className="rounded-lg bg-[var(--surface-elevated)]/70 overflow-hidden flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 py-3 border-b border-[var(--surface-subtle)]">
          <div className="flex min-w-0 flex-col">
            <span className="typography-ui-label text-foreground">{t('settings.aiyo.about.field.version')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{currentVersion}</span>
          </div>
          <div className="flex min-w-0 flex-col">
            <span className="typography-ui-label text-foreground">{t('settings.aiyo.about.field.openCodeVersion')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{openCodeVersion || t('settings.aiyo.about.state.unknown')}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {updateStore.checking && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="typography-meta">{t('settings.aiyo.about.state.checking')}</span>
              </div>
            )}
            ...
            {!updateStore.checking && !updateStore.available && !updateStore.error && (
              <span className="typography-meta text-muted-foreground">{t('settings.aiyo.about.state.upToDate')}</span>
            )}
            ...
            >
              {t('settings.aiyo.about.actions.checkForUpdates')}
            </Button>
          </div>
        </div>
