  ...
  const handleSaveAndReload = React.useCallback(async () => {
    ...
    try {
      ...
      await reloadOpenCodeConfiguration({
        message: t('settings.aiyo.opencodeCli.actions.restartingOpenCode'),
        mode: 'projects',
        scopes: ['all'],
      });
    } finally {
      ...
      <div className="mb-1 px-1">
        <div className="flex items-center gap-2">
          <h3 className="typography-ui-header font-medium text-foreground">
            {t('settings.aiyo.opencodeCli.title')}
          </h3>
          <Tooltip>
            ...
            <TooltipContent sideOffset={8} className="max-w-xs">
              {t('settings.aiyo.opencodeCli.tooltipPrefix')}
              {' '}
              <code className="font-mono text-xs">opencode</code>
              {t('settings.aiyo.opencodeCli.tooltipSuffix')}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <section className="px-2 pb-2 pt-0 space-y-0.5">
        <div data-settings-item="sessions.opencode-binary" className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex min-w-0 flex-col shrink-0">
            <span className="typography-ui-label text-foreground">{t('settings.aiyo.opencodeCli.field.binaryPath')}</span>
          </div>
          <div className="flex min-w-0 items-center gap-2 sm:w-[20rem]">
            <Input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder={t('settings.aiyo.opencodeCli.field.binaryPathPlaceholder')}
            ...
            <Button
              ...
              aria-label={t('settings.aiyo.opencodeCli.actions.browseAria')}
              title={t('settings.aiyo.opencodeCli.actions.browse')}
            >
        ...
        <div className="py-1.5">
          <div className="typography-micro text-muted-foreground/70">
            {t('settings.aiyo.opencodeCli.tipPrefix')}
            {' '}
            <span className="font-mono">OPENCODE_BINARY</span>
            {' '}
            {t('settings.aiyo.opencodeCli.tipMiddle')}
            {' '}
            <span className="font-mono">~/.config/aiyo/settings.json</span>
            {'.'}
          </div>
        </div>
        ...
        <label data-settings-item="sessions.opencode-update-notifications" className="flex cursor-pointer items-center gap-2 py-1.5">
          <Checkbox
            ...
            ariaLabel={t('settings.aiyo.opencodeCli.field.showUpdateNotificationsAria')}
          />
          <span className="typography-ui-label text-foreground">
            {t('settings.aiyo.opencodeCli.field.showUpdateNotifications')}
          </span>
        </label>
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.aiyo.opencodeCli.actions.saveAndReload')}
          </Button>
        </div>
      </section>
    </div>
