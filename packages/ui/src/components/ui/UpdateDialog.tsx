...
const GITHUB_RELEASES_URL = 'https://github.com/t880216t/aiyo/releases';
...
async function installWebUpdate(): Promise<InstallWebUpdateResult> {
  try {
    const response = await runtimeFetch('/api/aiyo/update-install', {
      method: 'POST',
...
async function waitForUpdateApplied(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const response = await runtimeFetch('/api/aiyo/update-check', {
        method: 'GET',
        headers: { Accept: 'application/json' },
      });
      if (response.ok) {
