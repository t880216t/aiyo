...
export const AgentManagerView: React.FC<AgentManagerViewProps> = ({ className }) => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () =>
      (typeof window !== 'undefined'
        ? (window as unknown as { __AIYO_CONNECTION__?: { status?: string } }).__AIYO_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as unknown as { __AIYO_CONNECTION__?: { status?: string } }).__AIYO_CONNECTION__?.status
