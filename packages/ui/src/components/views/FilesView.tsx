...
const FILE_EDITOR_AUTO_SAVE_KEY = 'aiyo:files:auto-save-enabled';
type FileLineEnding = '\n' | '\r\n';
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'aiyo:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'aiyo:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'aiyo:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('aiyo:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);
