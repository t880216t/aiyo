...
import {
  ...
  AIYO_PROJECT_NOTES_MAX_LENGTH,
  readProjectPlanFile,
  AIYO_PROJECT_TODO_TEXT_MAX_LENGTH,
  saveProjectNotesAndTodos,
  type AiYoProjectPlanFileLink,
  type AiYoProjectTodoItem,
  type ProjectRef,
} from '@/lib/aiyoConfig';
...
type ProjectPlanListItem = AiYoProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: AiYoProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: AiYoProjectTodoItem[]): AiYoProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: AiYoProjectTodoItem[], item: AiYoProjectTodoItem): AiYoProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<AiYoProjectTodoItem[]>([]);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: AiYoProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('aiyo:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('aiyo:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('aiyo:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleAddTodo = React.useCallback(() => {
    ...
    const nextTodos = insertTodoBeforeCompleted(todos, {
      id: createTodoId(),
      text: trimmed.slice(0, AIYO_PROJECT_TODO_TEXT_MAX_LENGTH),
      completed: false,
      createdAt: Date.now(),
    });
    setTodos(nextTodos);
  ...
  const todoInputValue = newTodoText.slice(0, AIYO_PROJECT_TODO_TEXT_MAX_LENGTH);
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewAiYoSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewAiYoSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('aiyo:project-plan-saved', {
    ...
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{notes.length}/{AIYO_PROJECT_NOTES_MAX_LENGTH}</span>
        </div>
        <Textarea
          value={notes}
          onChange={(event) => setNotes(event.target.value.slice(0, AIYO_PROJECT_NOTES_MAX_LENGTH))}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{todoInputValue.length}/{AIYO_PROJECT_TODO_TEXT_MAX_LENGTH}</span>
        </div>

        <div className="flex items-center gap-1.5">
          <Input
            value={todoInputValue}
            onChange={(event) => setNewTodoText(event.target.value.slice(0, AIYO_PROJECT_TODO_TEXT_MAX_LENGTH))}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddTodo();
