...
const AUTO_DISCOVER_ACTION_ID = '__aiyo_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<AiYoProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<AiYoProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: AiYoProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: AiYoProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<AiYoProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: AiYoProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: AiYoProjectAction, toggleStopIfRunning = false) => {
