...
const DESKTOP_BROWSER_INSPECT_SCRIPT = `new Promise((resolve) => {
  const existing = document.getElementById('__aiyo_desktop_browser_overlay');
  if (existing) existing.remove();
  if (typeof window.__aiyoDesktopBrowserCancelInspect === 'function') {
    try { window.__aiyoDesktopBrowserCancelInspect(); } catch { /* webview not ready */ }
  }
  const overlay = document.createElement('div');
  overlay.id = '__aiyo_desktop_browser_overlay';
  ...
  const cleanup = () => {
    ...
    if (window.__aiyoDesktopBrowserCancelInspect === cancel) {
      delete window.__aiyoDesktopBrowserCancelInspect;
    }
  };
  const cancel = () => {
    cleanup();
  ...
  window.__aiyoDesktopBrowserCancelInspect = cancel;
...
const DESKTOP_BROWSER_CANCEL_INSPECT_SCRIPT = `(() => {
  if (typeof window.__aiyoDesktopBrowserCancelInspect === 'function') {
    window.__aiyoDesktopBrowserCancelInspect();
    return;
  }
  const overlay = document.getElementById('__aiyo_desktop_browser_overlay');
  if (overlay) overlay.remove();
})()`;

const DESKTOP_BROWSER_SAME_WEBVIEW_NAVIGATION_SCRIPT = `(() => {
  if (window.__aiyoSameWebviewNavigationInstalled) return;
  window.__aiyoSameWebviewNavigationInstalled = true;
...
const PreviewPane: React.FC<PreviewPaneProps> = ({ rawUrl, onNavigate }) => {
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      if (!data || data.source !== 'aiyo-preview-bridge' || data.version !== 1) {
        return;
      }
...
const isElectronBrowserRuntime = (): boolean => {
  return typeof window !== 'undefined' && Boolean(window.__AIYO_ELECTRON__);
};
...
const IframeBrowserPane: React.FC<DesktopBrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  React.useEffect(() => {
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      const data = event.data;
      if (!data || data.source !== 'aiyo-preview-bridge' || data.version !== 1) return;
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        {iframeSrc ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-background p-6 text-center">
            <AiYoLogo width={140} height={140} className="opacity-20" />
    ...
    <div className="absolute inset-0 flex flex-col bg-background">
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        <webview
          ...
          partition="persist:aiyo-browser"
          allowpopups
  ...
  const postThemeSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      const directThemeSync = (frameWindow as unknown as {
        __aiyoApplyThemeSync?: (themePayload: typeof payload) => void;
      }).__aiyoApplyThemeSync;
  ...
  const postEmbeddedVisibilityToChats = React.useCallback(() => {
    ...
    for (const [tabID, frame] of chatFrameRefs.current.entries()) {
      ...
      const directVisibilitySync = (frameWindow as unknown as {
        __aiyoSetEmbeddedVisibility?: (visibilityPayload: typeof payload) => void;
      }).__aiyoSetEmbeddedVisibility;
