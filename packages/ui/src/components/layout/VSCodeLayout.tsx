...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __AIYO_CONNECTION__?: { status?: string } }).__AIYO_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('aiyo.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('aiyo.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __AIYO_CONNECTION__?: { status?: string } }).__AIYO_CONNECTION__?.status
    ...
    window.addEventListener('aiyo:connection-status', handler as EventListener);
    return () => window.removeEventListener('aiyo:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('aiyo:navigate', handler as EventListener);
    return () => window.removeEventListener('aiyo:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('aiyo_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[AiYo][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[AiYo][VSCode][bootstrap] post-load', {
