...
const codeBin = process.env.AIYO_VSCODE_BIN || 'code';
const workspaceArg = process.argv[2] || process.env.AIYO_VSCODE_DEV_WORKSPACE || repoRoot;
const workspacePath = path.resolve(workspaceArg);

const resolveDevServerAddress = () => {
  const configured = process.env.AIYO_VSCODE_WEBVIEW_URL;
  if (!configured) {
