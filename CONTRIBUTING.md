# Contributing to AiYo

## Getting Started

```bash
git clone https://github.com/t880216t/aiyo.git
cd aiyo
bun install
```
...
Both are configurable via env vars: `AIYO_PORT`, `AIYO_HMR_UI_PORT`, `AIYO_HMR_API_PORT`.
...
`bun run vscode:dev` opens an Extension Development Host automatically. You can override the editor or workspace with `AIYO_VSCODE_BIN` and `AIYO_VSCODE_DEV_WORKSPACE`.

Example: `AIYO_VSCODE_BIN=cursor bun run vscode:dev`.
...
| `bun run pack:web` | Create a package archive for `@aiyo/web` |
...
Open an [issue](https://github.com/t880216t/aiyo/issues) or ask in [Discord](https://discord.gg/ZYRSdnwwKA).
